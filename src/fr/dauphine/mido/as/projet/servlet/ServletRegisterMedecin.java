	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(insert.equals("ok")) {
            ...
            if(listeCentre.length == 1) {
            	this.mailContent = String.format("Bonjour %s %s,<br/><br/>Vous avez reçu ce courriel car vous avez été inscrit sur notre plateforme par un de nos administrateurs.<br/><br/>Votre centre affili� est %s.<br/><br/>Pour vous connecter, utilisez votre adresse mail \"%s\" et votre mot de passe \"%s\", que nous vous invitons � changer rapidement � des fins de s�curit�. <br/><br/>Nous esperons que notre service vous portera satisfaction.<br/><br/>Cordialement, l'�quipe", personne.getPrenom(), personne.getNom(), this.servicesCentre.getCentreById(Integer.parseInt(listeCentre[0])).getNom(), medecin.getEmail(), medecin.getMotDePasse());
            	...
            	this.mailContent = String.format("Bonjour %s %s,<br/><br/>Vous avez reçu ce courriel car vous avez été inscrit sur notre plateforme par un de nos administrateurs.<br/><br/>Vos centres affili�s sont %s.<br/><br/>Pour vous connecter, utilisez votre adresse mail \"%s\" et votre mot de passe \"%s\", que nous vous invitons � changer rapidement � des fins de s�curit�. <br/><br/>Nous esperons que notre service vous portera satisfaction.<br/><br/>Cordialement, l'�quipe", personne.getPrenom(), personne.getNom(), centres, medecin.getEmail(), medecin.getMotDePasse());
        	...
        	if(insert.equals("email")) {
        		...
        		request.setAttribute("warning", "Vous ne pouvez pas inscrire plusieurs fois le m�me m�decin dans un même centre !");
            }
        	else {
