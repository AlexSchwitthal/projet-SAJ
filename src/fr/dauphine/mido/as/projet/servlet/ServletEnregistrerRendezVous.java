	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(planningInserted != null) {
			...
			String mailContent = String.format("Bonjour,<br/><br/>Vous vous confirmons la prise de votre rendez-vous de " 
					+ planning.getHeureDebutString() + " � " + planning.getHeureFinString() + " le " + planning.getDate().toString() 
					+ " avec le " + speMedecin.getSpecialite().getLibelle() + " " + medecin.getPersonne().getPrenom() + " " + 
					medecin.getPersonne().getNom() + " au centre " + centre.getNom() + " situ� � l'adresse " +
					centre.getAdresse().getAdresseComplete() + ". Vous pouvez joindre le centre au num�ro suivant : " + centre.getTelephone() + "<br/><br/>Cordialement, l'�quipe");
