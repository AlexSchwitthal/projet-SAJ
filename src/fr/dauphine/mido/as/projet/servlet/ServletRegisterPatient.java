...
public class ServletRegisterPatient extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
	        ...
	        if(insert.equals("ok")) {
	            request.setAttribute("success", "Vous vous �tes bien inscrit !");
		        this.getServletContext().getRequestDispatcher("/jsp/login.jsp").forward(request, response);
		        this.mailContent = String.format("Bonjour %s %s,<br/><br/>Vous avez reçu ce courriel car vous vous êtes inscrit sur notre plateforme.<br/><br/> Vous pouvez désormais prendre des rendez-vous m�dicaux sur notre site. Nous esperons que notre service vous portera satisfaction.<br/><br/>Cordialement, l'�quipe", personne.getPrenom(), personne.getNom());
	        	...
	        	if(insert.equals("email")) {
		            request.setAttribute("warning", "L'adresse e-mail que vous avez saisie est d�j� prise !");
	        	}
	        	else { 
